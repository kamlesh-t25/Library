...
const Orders = () => {
  ...
  const { booksList, orders,returnBook } = useContext(StoreContext);
    ...
    <div className='ordersContainer'>
      ...
      </div>
      {
        orderedBooks.map((element, index) => {
          ...
          return (
            <div key={index} id={element._id} className={`bookCard ${orderItem.status === "Returned" ? "redOrder" : "greenOrder"}`} style={{border: orderItem.status === "Pending" ? "2px solid red" : "2px solid green"}}>
              <h2>{element.title}</h2>
              <p className='orders-authorName'>{element.author}</p>
              <h2 className={orderItem.status === "Pending" ? "redColor" : "greenColor"} style={{color:orderItem.status=="Returned" ? "blue":""}}>{orderItem.status}</h2>
              ...
              <button className='book-return-button' onClick={()=>returnBook(element._id)}>Return</button>
            </div>
          );
        })
      }
